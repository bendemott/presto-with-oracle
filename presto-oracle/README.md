...
| `oracle.number.type.default`                | `DECIMAL`, `DOUBLE`, `INTEGER`, `VARCHAR`        | `DECIMAL`   | Default type to convert Oracle `NUMBER` types to                  
|                                             |                                                  | `ROUND`     | Overidden by: `oracle.number.type.as-double`, `oracle.number.type.as-integer`, `oracle.number.type.as-varchar`,
|                                             |                                                  |             | `oracle.number.type.zero-scale-type`, `oracle.number.type.null-scale-type`                                                                                                             
...
| `oracle.number.decimal.round-mode`          | `CEILING`, `DOWN`, `FLOOR`,`HALF_EVEN`,          | `HALF_EVEN` | When a NUMBER is treated as a DECIMAL it will be rounded using this method
|                                             | `HALF_DOWN`, `HALF_UP`, `UP`, `UNNECESSARY`      |             | 
...
| `oracle.number.decimal.precision-map`       | `null:null=38:12`                                |             | 
| `oracle.number.double.default-scale.fixed`  | `1`, `4`, `8`                                    |             | When a `NUMBER` is treated as a `DOUBLE` it will be rounded to this scale
| `oracle.number.double.round-mode`           | `CEILING`, `DOWN`, `FLOOR`,`HALF_EVEN`,          | `HALF_EVEN` | When a `NUMBER` is treated as a `DOUBLE` it will be rounded using this method if needed
|                                             | `HALF_DOWN`, `HALF_UP`, `UP`, `UNNECESSARY`      |             | 
...
oracle.number.exceeds-limits=VARCHAR
oracle.number.type.zero-scale-type=INTEGER
```
